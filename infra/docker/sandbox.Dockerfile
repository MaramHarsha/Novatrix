...
# Builder: Go 1.25+ for ProjectDiscovery; official image sets GOTOOLCHAIN=local — override so @latest can
# fetch a newer toolchain if upstream bumps go.mod (avoids surprise version errors mid-build).
FROM golang:1.25-bookworm AS gobin
ENV GOTOOLCHAIN=auto
ENV CGO_ENABLED=0

# libpcap headers + runtime linker deps for naabu (gopacket/pcap); gcc is bundled in official golang images.
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
    libpcap-dev \
    && rm -rf /var/lib/apt/lists/*

# --- ProjectDiscovery (httpx/subfinder/dnsx/… are pure Go; katana + naabu need CGO + libpcap) ---
...
# Smoke test — validates copied binaries + libc (naabu links libpcap); ferox/rustscan may still be absent on some arches
CMD ["/bin/bash", "-lc", "nuclei -version && httpx -version && naabu -version && echo ok"]
